...
> 已排序的数组用第一个元素做 pivot → 直接退化为 `O(n²)`！

**平均复杂度证明思路**：基准等概率随机选 → 任意两个元素比较概率 = `2/(j-i+1)`
→ 总比较次数期望 = `O(n log n)`
...
> 复杂度 `O(n log n)`，`n ≤ 5 × 10⁵` 可过。注意答案可能到 `n²/2`，需用 `long long`！
...
| **时间** | `O(n log n)` | `Θ(n log n)` | `O(d(n+r))` |
| **最坏** | `O(n²)` | `Θ(n log n)` | `O(d(n+r))` |
| **空间** | `O(log n)` | `O(n)` | `O(n+r)` |
| **稳定性** | ❌ 不稳定 | ✅ 稳定 | ✅ 稳定 |
| **sort() 实现** | Introsort | — | — |
